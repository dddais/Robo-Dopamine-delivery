...
4. 后续新帧只运行 tracker；周期性或丢失后重新检测。更新期间，GRM 仍可读取上一组已完成且未过期的结果。
...
- `redetect_interval_s: 5.0`：定期重新检测，并优先匹配 tracker 当前实例的位置。找不到匹配目标时丢弃旧框，不直接跳到远处的高分同类物体。
- `max_gap_s: 2.0`：两次更新之间长时间断帧后重新检测。
- `min_score: 0.5`：跟踪目标存在分数过低或 mask 为空时重新检测；它与文本检测置信分数不同，UI 会区分来源。
- `match_iou: 0.1`：周期性重检测与本帧跟踪框的最小匹配 IoU。
...
每个任务 generation、每个摄像头有独立 session；任务结束/stop 时释放，下一轮重新初始化。reset 后重新开始任务也会重新检测。初始多候选歧义不会强行选定目标。遮挡后重新检测仍不能保证物理实例身份绝不发生混淆，需要在连续真机视频上评估。

Tracker 只在服务端保存状态，不使用图像检测的持久缓存。网络失败时该帧返回明确的缺失定位，按现有 `on_missing_bbox` 策略处理；不会套用上一帧 bbox。
...
正常持续评分时没有 FIFO 积压，但延时不是常数，会随负载、重检测、传输和轮询波动。服务停顿后，旧结果仍会越来越旧，不能将其视为当前机器人状态。
